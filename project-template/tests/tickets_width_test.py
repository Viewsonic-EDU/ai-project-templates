"""Ticket-board width regression: run real ticket functions against an isolated, disposable git store."""
import argparse
import errno
import fcntl
import os
...
def width(text):
    return sum(0 if any(a <= ord(c) <= b for a, b in ZERO) else
               2 if any(a <= ord(c) <= b for a, b in WIDE) else 1 for c in text)
...
def env_for(cols=120, color="never"):
    ...
    for key in list(env):
        if key.startswith(("TICKETS_", "GIT_")) or key in ("BASH_ENV", "ENV"):
            env.pop(key)
    env.update(TICKET_PREFIX="TKT", TICKETS_COLOR=color,
               TICKETS_DONE_LIMIT="0", TICKETS_COLS=str(cols), TERM="xterm-256color")
    return env


def shell(repo, code, data=None, cols=120, legacy=False, color="never"):
    source = 'source ./tickets.sh\n' + (LEGACY_CELL if legacy else "")
    return subprocess.run(["bash", "-c", source + code], cwd=repo,
                          env=env_for(cols, color), input=data,
                          stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                          check=True).stdout


def cells(repo, cases, legacy=False):
    data = b"".join(text.encode() + b"\0" + str(w).encode() + b"\0"
                    for text, w in cases)
    out = shell(repo, '''while IFS= read -r -d '' text && IFS= read -r -d '' w; do
  cell "$text" "$w"
  printf '\\0'
done
''', data, legacy=legacy)
...
def unit_tests(repo):
    texts = ["", "hello", "中文", "A中文B", "ASCII title " * 8,
             "中文標題" * 15, "A中B文 mixed " * 12,
             "e\u0301", "中\u0301文", "a\u200bb\u200fc\ufeff", "\u0301\u200b",
             "↳ TKT-4 中文", "😀🦊𠀀", "a\\b literal $value"]
    ...
    for (text, w), result in zip(cases, actual):
        ...
        for color in ("never", "always"):
            for command in ("cmd_list", "cmd_board"):
                assert shell(repo, command, cols=cols, color=color) == shell(
                    repo, command, cols=cols, color=color, legacy=True), (command, cols, color)
    fixture(repo)
    print("PASS board: four exact-width CJK cells, child prefix, colors; list/board ASCII byte compatibility")
...
def watch_capture(repo, rows, cols, artifact_dir=None):
    ...
    if pid == 0:
        ...
        assert struct.unpack("HHHH", fcntl.ioctl(
            0, termios.TIOCGWINSZ, b"\0" * 8))[:2] == (rows, cols)
    ...
    try:
        ...
        print(f"PASS PTY {rows}x{cols}: 4 frames, each model item once, no row overflow, selected child exact width")
    finally:
